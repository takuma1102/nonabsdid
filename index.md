...
This is **experimental** and currently supports **DCDH** and the
**fect** family only. **PanelMatch is intentionally omitted**: a
faithful cohort matrix needs per-cohort estimates *and* per-cohort SEs,
and for PanelMatch that requires re-aggregating matched-set effects by
switch time and re-running the matched-set bootstrap on that
re-aggregation — out of scope for now, so it is left out rather than
shipped with incorrect standard errors. Compare methods as triangulation
of the *pattern*, not as cell-by-cell equality (the estimators differ in
estimand, controls, and coverage). See the [*Cohort-by-time effect
matrices and
